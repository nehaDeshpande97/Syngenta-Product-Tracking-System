...
$query = "SELECT * FROM dbo.CartonItem WHERE CartonSGTINID IN ('0118964001684104217G6TQQ7P48','011896400168410421AYP684T2H1')";
...
while( $row = sqlsrv_fetch_array( $result, SQLSRV_FETCH_ASSOC) ) {
      $CartonItemSGTINID[$i]= $row['CartonItemSGTINID'];
       $CartonSGTINID[$i]= $row['CartonSGTINID'];
      //$CartonGTIN[$i]= $row['CartonGTIN'];
...
$i=0;
$quantity=1;
$FromAddress="1UyKVTLCj4AuQyVkeVMvobkBRMLGrhaRddTgeN";
$addresses="1UyKVTLCj4AuQyVkeVMvobkBRMLGrhaRddTgeN";
$multiple=1;
		//print("In If");
...
*/
		$custom['CartonSGTINID'] =(string) $CartonSGTINID[$i];
		//$custom['CartonItemSGTINID'] =(string) $CartonItemSGTINID[$i];
		//$custom['BrandName'] =(string) $BrandName[$i];
		$custom['Quantity'] =(string) $quantity;
     
		$datas=array( // to create array of data items
			array( // metadata for issuance details
				'name' => $CartonItemSGTINID[$i],
				//'multiple' => $multiple,
				'open' => true,
				'details' => $custom,
			)
		...
		$success=no_displayed_error_result($issuetxid, multichain('createrawsendfrom', $FromAddress, $addresses, $datas, 'send'));
...
?>
						<table class="table table-bordered table-condensed  <?php echo ($success && ($name==@$_POST['name'])) ? 'bg-success' : 'table-striped'?>">
							<tr>
								<th style="width:65%;">ItemSGTIN</th>
...
?>
			...
			<div class="form-group">
							<div class="col-sm-offset-2 col-sm-9 text-center">
								<input class="btn btn-primary btn-md" style="background-color: #00a0be;border:none" type="submit" name="issueasset" value="Issue Asset">
							</div>
							</div>
						</form>
